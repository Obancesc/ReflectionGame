...
MainMenu.prototype = 
{
	...
	create: function()
	{
		...
		uiSound = game.add.audio('rain');
		var button = game.add.image(43, 248, 'button');
		button.scale.setTo(3,1.8);
		titleText = game.add.text(70, 275, 'Press ENTER to play', {font: '29px Segoe Print', fill: '#000'});
		var tween = game.add.tween(titleText).to({width: 320, height: 55}, 1575, 'Linear', true, 0, -1, true);
		var tween2 = game.add.tween(button).to({width: 375, height: 110}, 1570, 'Linear', true, 0, -1, true);
	},
	update: function()
	{
...
GameOver.prototype=
{
	...
	create:function()
	{
		...
		var tween = game.add.tween(scoreText).to({width: 448, height: 60}, 1675, 'Linear', true, 0, -1, true);
	},
	update: function()
	{
		if(game.input.keyboard.isDown(Phaser.Keyboard.ENTER))
