...
create: function() 
{
	...
	//lakeBackground
	mirrorBackground = game.add.tileSprite(0, 525, 1024, 1024, 'mirrorBackground');
	//mirror= game.add.tileSprite(0, 500, 1024, 1024, 'mirror');
	//mirrorGoodPlatforms
...
update: function() 
{
	...
	var fakePlatform= game.physics.arcade.overlap(player, badPlatforms);
	console.log(timeDelay);
	if(timeDelay!=0)
	{
		mirrorBackground.y-=.7;
		mirror.y-=.7;
	}
	...
	{
		timeDelay=0;
		game.time.events.add(Phaser.Timer.SECOND * .5, raiseWater, this);
		score+=1;
...
function makePlatforms()
{
	for(var i=0; i<1; i++)
	{
		...
		mirror.y+=60;
		mirrorBackground.y+=60;
		if(mirror.y>525)
		{
			mirror.y=500;
			mirrorBackground.y=525;
		}
	}
}
